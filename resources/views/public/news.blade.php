@extends('layouts.app')
@section('title', __('public.news_page.title'))
@section('meta_description', __('public.news_page.meta'))
...
<section class="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
    <div class="max-w-3xl">
        ...
        <h1 class="mt-5 text-4xl font-black tracking-[-0.02em] text-slate-950 sm:text-5xl">{{ __('public.news_page.heading') }}</h1>
        <p class="mt-5 text-lg leading-8 text-slate-600">{{ __('public.news_page.subtitle') }}</p>
    </div>

    @if($items->isEmpty())
        <div class="mt-12 rounded-3xl border border-dashed border-slate-300 bg-white p-12 text-center text-slate-400">
            {{ __('public.news_page.empty') }}
        </div>
    @else
            ...
            @foreach($items as $item)
                <a href="{{ route('news.show', $item->slug) }}" class="group flex flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-0.5 hover:border-blue-200 hover:shadow-xl">
                    ...
                    <div class="flex flex-1 flex-col p-5">
                        ...
                        <h2 class="mt-2 text-lg font-bold leading-snug text-slate-950 group-hover:text-blue-700">{{ $item->title }}</h2>
                        <p class="mt-2 line-clamp-3 text-sm leading-6 text-slate-500">{{ $item->excerpt ?: \Illuminate\Support\Str::limit(strip_tags($item->body), 130) }}</p>
                        <span class="mt-4 inline-flex items-center gap-1 text-sm font-semibold text-blue-600">{{ __('public.news_page.read') }} <x-icon name="arrow-right" class="h-4 w-4 transition group-hover:translate-x-0.5" /></span>
                    </div>
                </a>
            @endforeach
        </div>
