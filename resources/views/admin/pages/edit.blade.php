    ...
    <form method="POST" action="{{ route('admin.pages.update', $page) }}" class="grid gap-6 xl:grid-cols-[1fr_0.4fr]" x-data="{ lang: 'uk' }">
        ...
        <x-card>
            ...
            <div x-show="lang==='ru'" x-cloak class="space-y-4">
                <div><label class="fin-label">Заголовок (RU)</label><input name="title_ru" type="text" class="fin-input" value="{{ old('title_ru', $page->title_ru) }}"></div>
                <div><label class="fin-label">Текст (RU) <span class="text-slate-400">(HTML)</span></label><textarea name="body_ru" rows="16" class="fin-input font-mono text-sm">{{ old('body_ru', $page->body_ru) }}</textarea></div>
                <div><label class="fin-label">Meta title (RU)</label><input name="meta_title_ru" type="text" class="fin-input" value="{{ old('meta_title_ru', $page->meta_title_ru) }}"></div>
                <div><label class="fin-label">Meta description (RU)</label><textarea name="meta_description_ru" rows="2" class="fin-input">{{ old('meta_description_ru', $page->meta_description_ru) }}</textarea></div>
            </div>
