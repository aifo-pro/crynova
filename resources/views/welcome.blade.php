...
<section class="mx-auto max-w-6xl px-4 py-24 sm:px-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
            <p class="text-sm font-bold uppercase tracking-[0.18em] text-blue-600">{{ __('public.blog_page.badge') }}</p>
            <h2 class="mt-3 max-w-xl text-4xl font-black tracking-[-0.03em] text-slate-950">{{ __('public.home.blog_title') }}</h2>
        </div>
        <a href="{{ lroute('blog') }}" class="inline-flex shrink-0 items-center gap-2 self-start rounded-full border border-slate-200 px-5 py-2.5 text-sm font-bold text-slate-900 transition hover:border-blue-200 hover:text-blue-600 sm:self-auto">
            {{ __('public.home.blog_all') }} <x-icon name="arrow-right" class="h-4 w-4" />
        </a>
    </div>

    <div class="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        @foreach($latestPosts as $post)
            <a href="{{ lroute('blog.show', ['post' => $post->slug]) }}" class="group flex flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:border-blue-200 hover:shadow-xl hover:shadow-slate-200 {{ $loop->last ? 'sm:max-lg:hidden lg:flex' : '' }}">
                @if($post->cover_image)
                ...
                <div class="flex flex-1 flex-col p-6">
                    <div class="flex items-center gap-3 text-xs font-semibold text-slate-400">
                        <span class="text-blue-600">{{ optional($post->published_at)->translatedFormat('d F Y') }}</span>
                        <span class="inline-flex items-center gap-1"><x-icon name="clock" class="h-3.5 w-3.5" /> {{ $post->readingMinutes() }} {{ __('public.blog_page.read_time') }}</span>
                    </div>
                    <h3 class="mt-3 text-lg font-bold leading-snug text-slate-950 transition group-hover:text-blue-700">{{ $post->tr('title') }}</h3>
                    <p class="mt-2 line-clamp-3 flex-1 text-sm leading-6 text-slate-500">{{ $post->tr('excerpt') ?: \Illuminate\Support\Str::limit(strip_tags($post->tr('body')), 130) }}</p>
                    <span class="mt-5 inline-flex items-center gap-1 text-sm font-semibold text-blue-600">{{ __('public.blog_page.read') }} <x-icon name="arrow-right" class="h-4 w-4 transition group-hover:translate-x-0.5" /></span>
                </div>
            </a>
        @endforeach
    </div>
