@extends('layouts.app')
@section('title', __('ui.auth.reset_page'))
...
<div class="mx-auto flex min-h-[70vh] max-w-md items-center px-4">
    <div class="w-full rounded-[2rem] border border-slate-200 bg-white p-8 shadow-xl shadow-slate-200/70">
        <h1 class="text-center text-2xl font-semibold text-slate-950">{{ __('ui.auth.reset_title') }}</h1>
        <p class="mt-2 text-center text-sm leading-6 text-slate-500">{{ __('ui.auth.reset_sub') }}</p>
        ...
        <form method="POST" action="{{ route('password.update') }}" class="mt-6 space-y-5">
            ...
            <div>
                <label class="fin-label">{{ __('ui.auth.new_password') }}</label>
                <input name="password" type="password" required class="fin-input" autocomplete="new-password">
            </div>
            <div>
                <label class="fin-label">{{ __('ui.auth.repeat_password') }}</label>
                <input name="password_confirmation" type="password" required class="fin-input" autocomplete="new-password">
            </div>
            <x-button type="submit" class="w-full rounded-full py-3">{{ __('ui.auth.update_password') }}</x-button>
        </form>
    </div>
</div>
@endsection
